...
//--------------READINGS------------------//
 ...
 void readSensor() {
  ...
  float accelXArray[20];
  float accelYArray[20];
  float accelZArray[20];
  ...
  for (j=0; j < 20; j++) {
    ...
    accelReal.x = aReal_current_x_sum/10;
    accelReal.y = aReal_current_y_sum/10;
    accelReal.z = aReal_current_z_sum/10;
  
    accelWorld.x = aWorld_current_x_sum/10;
    accelWorld.y = aWorld_current_y_sum/10;
    accelWorld.z = aWorld_current_z_sum/10;

    accelXArray[j] = (accelReal.x - accelWorld.x)*factor_conversion;
    accelYArray[j] = (accelReal.x - accelWorld.x)*factor_conversion;
    accelZArray[j] = (accelReal.x - accelWorld.x)*factor_conversion;
  ...
  for (k=0;k<20;k++) {
    ...
    accelXStr += accelXArray[k];
    accelYStr += accelYArray[k];
    accelZStr += accelZArray[k];
  ...
  accelXStr += ":";
  Serial.print(accelXStr);
  Serial.flush();
  accelYStr += ":";
  Serial.print(accelYStr);
  Serial.flush();
  Serial.print(accelZStr);
  Serial.flush();
  Serial.print(";");
  Serial.flush();
  Serial.print(quaternionData);
  Serial.flush();
  Serial.print("\n");
  Serial.flush();
...
void loop() {
  // if programming failed, don't try to do anything
    ...
    readSensor(); //This actually sends too
